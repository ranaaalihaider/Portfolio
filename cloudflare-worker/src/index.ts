...
function base64UrlEncode(str: string) { return btoa(str).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); }
...
function getKarachiStartOfMonth(): string {
...
function isValidPeriod(p: string) {
  return ['today', '7', '30'].includes(p);
}

function validateSql(sql: string): { isValid: boolean; error?: string; cleanSql?: string } {
  // Trim and remove markdown
  ...
  // Reject multiple statements (allow trailing semicolon)
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const respondJSON = (data: any, status = 200) => new Response(JSON.stringify(data), { status, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
    const errorResponse = (msg: string, status = 400) => respondJSON({ success: false, error: msg }, status);
    ...
    if (pathname === '/api/ai/sql' && request.method === 'POST') {
      ...
      const customer: any = await env.DB.prepare('SELECT id, name, status, monthly_limit FROM customers WHERE api_key_hash = ?').bind(keyHash).first();
      ...
      const usageResult: any = await env.DB.prepare('SELECT COUNT(*) as count FROM request_logs WHERE customer_id = ? AND status = "success" AND request_date >= ?')
...
Return SQL only. No markdown. No explanation. Never generate INSERT, UPDATE, DELETE, DROP, ALTER, CREATE, TRUNCATE, REPLACE, GRANT or REVOKE. Generate exactly one read-only statement.`
    ...
    if (pathname.startsWith('/api/admin/')) {
      ...
      if (pathname === '/api/admin/dashboard' && request.method === 'GET') {
        ...
        return respondJSON({
          totalCustomers: (totalCust as any)?.count || 0,
          activeCustomers: (activeCust as any)?.count || 0,
          blockedCustomers: (blockCust as any)?.count || 0,
          requestsToday: (reqToday as any)?.count || 0,
          requestsThisMonth: (monthStats as any)?.count || 0,
          inputTokensThisMonth: (monthStats as any)?.in_tok || 0,
          outputTokensThisMonth: (monthStats as any)?.out_tok || 0,
          totalTokensThisMonth: (monthStats as any)?.tot_tok || 0,
          recentUsage: recent.results.map(r => ({ date: r.request_date, requests: (r as any).count })).reverse()
        });
      }
      ...
      if (pathname === '/api/admin/customers' && request.method === 'POST') {
        const { name, monthly_limit } = await request.json() as any;
      ...
      if (custMatch) {
        ...
        if (request.method === 'GET') {
          ...
          return respondJSON({
            ...cust,
            requests_today: (reqToday as any)?.count || 0,
            requests_month: (mStats as any)?.count || 0,
            tokens_month: (mStats as any)?.tok || 0,
      ...
      const requestsMatch = pathname.match(/^\/api\/admin\/customers\/(\d+)\/requests$/);
      if (requestsMatch && request.method === 'GET') {
        ...
        const requests = await env.DB.prepare('SELECT id, request_date, request_time, input_tokens, output_tokens, total_tokens, status FROM request_logs WHERE customer_id=? ORDER BY id DESC LIMIT ? OFFSET ?').bind(id, limit, offset).all();
      ...
      if (pathname === '/api/admin/usage' && request.method === 'GET') {
        ...
        return respondJSON({
          totalRequests: (totals as any)?.c || 0,
          inputTokens: (totals as any)?.i || 0,
          outputTokens: (totals as any)?.o || 0,
          totalTokens: (totals as any)?.t || 0,
          daily: daily.results
        });
      }
    }
